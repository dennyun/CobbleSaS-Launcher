...
public class AsyncAssetManager {
    ...
    public static void unpackComponents(Context ctx){
        ...
        sExecutorService.execute(() -> {
            ...
            if (!prefs.getBoolean("jre_symlink_fixed", false)) {
                Log.i("AssetUnpacker", "Forcing a one-time clean JRE extraction due to symlink fix...");
                try {
                    org.apache.commons.io.FileUtils.deleteDirectory(new File(Tools.DIR_GAME_HOME, "caciocavallo"));
                    org.apache.commons.io.FileUtils.deleteDirectory(new File(Tools.DIR_GAME_HOME, "caciocavallo17"));
                    prefs.edit().putBoolean("jre_symlink_fixed", true).apply();
                } catch (Exception e) {
                    Log.e("AssetUnpacker", "Failed to clean JRE directory", e);
                }
            }
